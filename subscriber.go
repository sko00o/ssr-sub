...
func CheckNode(node *Config, c CheckConfig) bool {
	if matched, _ := regexp.MatchString(c.Not, node.Remarks); matched {
		log.Warnf("remarks %s not allowed, ignore", node.Remarks)
		return false
	}

	duration, err := time.ParseDuration(c.TCPTimeout)
	if err != nil {
		log.Error(err)
		return false
	}
	...
	if err != nil {
		log.Warnf("check %s:%d, failed", node.Server, node.ServerPort)
		return false
	}

	log.Infof("check %s:%d, passed", node.Server, node.ServerPort)
	return true
}
